# Deploy 0.3.27 to the astrotown rig. Run ON the rig (elevated ssh session).
#
# Derived from deploy_0326.ps1. What 0.3.27 carries:
#   * POLAR: a fit whose axis cannot reproduce the rotation the mount was
#     commanded to make is refused instead of published. On 2026-09-09 two
#     runs an hour apart, on different fields, each reported about 500' on a
#     mount that is really ~33' out; both bent by ~16' and no existing guard
#     could see it. Also 5 s of settle after each slew (an unproven
#     mitigation, stated as such), and the simulator now rotates by what it
#     was told rather than a fixed 15 degrees.
#   * everything already live in 0.3.26 is unchanged.
#
# The wheel step and the config step are kept from 0.3.26 and are both
# idempotent: 0.3.27 changes no Rust, so the wheel reinstalled here is the
# same one already in the venv and the peek_next check simply re-proves it,
# and sky_fallback_hold is already true so the config round-trip is a no-op
# that still verifies the file survives a rewrite.
#
# Every gate from 0.3.26 stays: the signed tarball hash, the idle-rig
# precheck, kill by explicit PID, the vendor carry-forward and its integrity
# check, the webui refresh, the staged-tree version, the pointer flip with
# rollback, and /healthz reporting the new version.
...
if ($staged.Line -notmatch '0\.3\.27') { throw "staged tree is not 0.3.27" }
$checks = @(
    @{ File = 'serverstrodeck\polar
ative.py';          Pattern = 'MAX_ROTATION_DISAGREEMENT_DEG';   Name = 'polar: the rotation-agreement guard' },
    @{ File = 'serverstrodeck\polar
ative.py';          Pattern = '_refuse_if_the_fit_does_not_reproduce_the_rotation'; Name = 'polar: the guard is wired in' },
    @{ File = 'serverstrodeck\polar
ative.py';          Pattern = '_SETTLE_AFTER_SLEW_S';             Name = 'polar: settle after each slew' },
    @{ File = 'serverstrodeckocus\window.py';          Pattern = 'def measure_window';               Name = 'autofocus measures a centred window' },
    @{ File = 'serverstrodeckocus\pipeline.py';        Pattern = 'peek_next';                        Name = 'autofocus predictor asks the engine' },
    @{ File = 'serverstrodeckocuspproach.py';        Pattern = 'async def approach';               Name = 'one-side approach rule' },
    @{ File = 'serverstrodeck\hub.py';                    Pattern = 'async def pier_side_now';          Name = 'flip checks the pier side before paying' },
    @{ File = 'serverstrodeck\sequence\engine.py';       Pattern = '_recentre_after_unguided_focus';   Name = 're-centre after the unguided sweep' },
    @{ File = 'serverstrodeck\dawn_park.py';              Pattern = '_release_cooler';                  Name = 'dawn releases the cooler' },
    @{ File = 'serverstrodeck\sequence\engine.py';       Pattern = 'GUIDE_CALIBRATE_TIMEOUT_S';        Name = 'guide-start bound covers a calibration' }
)
foreach ($c in $checks) {
...
if ($h -notmatch '0\.3\.27') { throw "the running server is not 0.3.27: $h" }
