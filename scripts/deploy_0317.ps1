...
$want = "index-CSYX3Rhz.js"
$idx = (& curl.exe -s --max-time 15 "http://127.0.0.1:8800/")
if ($idx -match 'assets/(index-[A-Za-z0-9_-]+\.js)') {
    $got = $Matches[1]
    Write-Output ("   served: " + $got + "  expected: " + $want)
    if ($got -ne $want) { throw ("served asset " + $got + " is not the build we shipped") }
} else {
    Write-Output "   NOTE: index.html not readable from 127.0.0.1; verify via the relay"
}
