"""Verify 0.3.31 actually carries what it claims, on the rig.

Derived from verify_0330.py. 0.3.31 = 0.3.30 + the flows compiler notes, the
...
print("== 3b. the running server answers 0.3.31 and its meridian block carries the new fields ==")
code, hz = call(cookie, "GET", "/healthz")
hz = hz if isinstance(hz, dict) else {}
v_ok = hz.get("version") == "0.3.31"
if not v_ok:
    ok = False
...
print("VERDICT:", "0.3.31 CARRIES THE FIXES" if ok else "SOMETHING IS MISSING")
print("VERIFY_DONE")
