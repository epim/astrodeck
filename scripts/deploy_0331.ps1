# Deploy 0.3.31 to the astrotown rig. Run ON the rig (elevated ssh session).
#
# What 0.3.31 carries, on top of 0.3.30: the flows compiler's ten node-settings
# notes become notes (not warnings) with what is carried, what is not and where
# the real value lives; the flows inspector shows them as one FROM THE RIG panel;
# the Sky tab opens on the atlas populated with tonight's targets, with SURVEY
# as a layer and a tap on empty sky aiming the reticle; the skydome draws
# tracks to dawn for the lock, the ranked targets and an aimed point; and the
# rig session's F3/F4/F6 (the engine refuses to expose while a meridian flip is
# owed - safety.flip_owed_hold_min 20; a guide loop with nobody driving it is
# stopped - safety.unattended_guide_min 10; pier side is published on
# status.meridian with source and age, and the AM5 driver reports the
# destination pier side, which arms safety.enforce_pier_limits). The two loops
# (temp comp, dew) still ship DISABLED. The user approved the wide release.
#
# All 0.3.30 gates stay. previous -> 0.3.30 keeps the rollback.
...
if ($staged.Line -notmatch '0\.3\.31') { throw "staged tree is not 0.3.31" }
$checks = @(
...
if (-not (Select-String -Path $ui.FullName -Pattern '0\.3\.31' -Quiet)) { throw "the built UI does not carry the 0.3.31 version stamp" }
Write-Output "   UI bundle carries 0.3.31"
...
if ($h -notmatch '0\.3\.31') { throw "the running server is not 0.3.31: $h" }
