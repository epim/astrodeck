...
# dawn even when a session is armed to resume (41e31797, issue #35: 15.5 h of
# TEC runtime were being spent to save a 4 minute cool-down), and the
# photosphere issue pass in the new UI (issues #38, #41, #42, #46, #48, #49,
# #50, #51, #52, #57, #58, #59, #75; the scanner's stillness witness, cues,
# aim cone and horizon tracer). No other server behaviour changes. previous
# -> 0.3.32 keeps the rollback. Built from a clean worktree of 0f41ef87 plus
# the one-line horizon.tsx patch (sweep.centreColumns() at line 266, #78)
# that a clean checkout needs to type-check; the tarball sha256 begins
# 45acb102f984ef9a. Staged 2026-09-19 07:29 PDT, mount parked, rig idle.
#
# All 0.3.32 gates stay, plus the #35 marker.
...
if ($staged.Line -notmatch '0\.3\.33') { throw "staged tree is not 0.3.33" }
$checks = @(
...
if (-not (Select-String -Path $ui.FullName -Pattern '0\.3\.33' -Quiet)) { throw "the built UI does not carry the 0.3.33 version stamp" }
Write-Output "   UI bundle carries 0.3.33"
...
if ($h -notmatch '0\.3\.33') { throw "the running server is not 0.3.33: $h" }
