# Deploy 0.3.13 - the cloud model gets a viewer: a tilted sky dome on Monitor.
...
# that had in fact worked (relay-verified index-C_LJV5nK.js afterwards).
$want = "index-C_LJV5nK.js"
$idx = ((& curl.exe -s --max-time 15 "http://127.0.0.1:8800/") -join [char]10)
if ($idx -match 'assets/(index-[A-Za-z0-9_-]+\.js)') {
    $got = $Matches[1]
