# Deploy 0.3.20 - measure where the guide scope actually points.
...
Write-Output "== verify what the running server SERVES =="
$want = "index-BlXBmB6i.js"
$idx = (& curl.exe -s --max-time 15 "http://127.0.0.1:8800/")
if ($idx -match 'assets/(index-[A-Za-z0-9_-]+\.js)') {
    Write-Output ("   served: " + $Matches[1] + "  expected: " + $want)
    if ($Matches[1] -ne $want) { throw ("served asset " + $Matches[1] + " is not this build") }
} else { throw "could not read the served asset" }
