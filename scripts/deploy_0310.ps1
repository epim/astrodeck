...
Write-Output "== server-only release: prove the FIX shipped (asset hash cannot) =="
$cfg = Join-Path $Rel "server\astrodeck\config.py"
$hit = Select-String -Path $cfg -Pattern 'def set_cloudmap'
if ($hit) { Write-Output ("   cloudmap setter present: " + $hit.Line.Trim()) }
else       { throw "set_cloudmap is NOT in the staged config.py" }
