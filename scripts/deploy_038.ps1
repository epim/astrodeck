...
#
# THE GUARD IN 037 IS DELIBERATELY NOT A REFUSAL HERE. 037 threw if a sequence
# was running, which is right when the operator can simply stop it first. On
# 2026-08-24 that was not possible: three consecutive aborts each reported
# {"aborted": true} and published state "aborted", and auto-resume restarted the
# same session within ~45 s every time - including after PATCH
# /api/sessions/<id> {auto_resume:false} returned 200 with auto_resume False.
# /api/sequence/resume-arm reported {"armed": null} throughout. So the run could
# not be stopped through any documented control, and a refusal here would mean
# the box can never be deployed to while that state persists.
#
# The abort below is therefore best-effort and is followed IMMEDIATELY by the
# kill, to win the race against the resume tick rather than wait for a state
# that never settles. The operator authorised interrupting this run.
...
Write-Output "== best-effort abort, then kill without waiting (see header) =="
...
try {
    ...
    Write-Output ("   sequence state before: " + $q.state)
    & curl.exe -s --max-time 20 -X POST -H $hdr -H "Content-Type: application/json" `
        -d "{}" "http://127.0.0.1:8800/api/sequence/abort" | Out-Null
    Write-Output "   abort posted"
} catch { Write-Output "   abort attempt: $_" }
...
$idx = (& curl.exe -s --max-time 15 "http://127.0.0.1:8800/")
