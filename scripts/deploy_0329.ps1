...
# the legacy UI kept at #/classic) and server wave S7 - satellite and comet
# ephemerides, the per-channel stack preview, the promote-last-frame route,
# SER video capture with a lucky stack, focuser temperature compensation, the
# dew-margin heater loop, the mount nudge verb and the driver's real slew
# ceiling, per-port run protection, aperture and reducer on the rig's optics,
# and the planning store that takes quick defaults, the target pool and the
# site pick off the phone. Then a nine-reviewer whole-branch review and its
# fixes. The two new loops (temp comp, dew) ship DISABLED; port protection
# defaults to the same name heuristic that runs today. The one behaviour that
# changes on its own: the touch pad may slew at the driver's 1.44 deg/s
# instead of 0.6, with the 1.2 s deadman unchanged.
#
# All 0.3.28 gates stay. The sky_fallback_hold config step is gone: 0.3.28
# already persisted it and the value lives in the config file.
...
if ($staged.Line -notmatch '0\.3\.29') { throw "staged tree is not 0.3.29" }
$checks = @(
...
if (-not (Select-String -Path $ui.FullName -Pattern '0\.3\.29' -Quiet)) { throw "the built UI does not carry the 0.3.29 version stamp" }
Write-Output "   UI bundle carries 0.3.29"
...
if ($h -notmatch '0\.3\.29') { throw "the running server is not 0.3.29: $h" }
