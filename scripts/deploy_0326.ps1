...
#   * the NATIVE WHEEL: the tarball ships no Rust engine, and 0.3.26's autofocus
#     predictor asks the engine for its next position through
#     FocusSweep.peek_next. An older wheel leaves the predictor sitting still
#     (safe, but the 7-minute sweep stays 7 minutes). The wheel is installed
#     into the venv while the server is STOPPED, because the loaded .pyd is
#     locked while it runs, and the running server is then asked whether it
#     has peek_next.
#   * the CONFIG: safety.sky_fallback_hold goes back to true (it was switched
#     off on the rig while the cloud hold could recurse; 0.3.26 carries the
#     fix). Set by a Python round-trip while the server is stopped -- never by
#     ConvertTo-Json, which collapses one-element arrays and once emptied the
#     user store.
#   * the checks list names the 0.3.26 changes rather than the 0.3.25 ones.
#
# Every gate from 0.3.25 stays: the signed tarball hash, the idle-rig precheck,
# kill by explicit PID, the vendor carry-forward and its integrity check, the
# webui refresh, the staged-tree version, the pointer flip with rollback, and
# /healthz reporting the new version.
...
if ($staged.Line -notmatch '0\.3\.26') { throw "staged tree is not 0.3.26" }
$checks = @(
    @{ File = 'server\astrodeck\focus\window.py';            Pattern = 'def measure_window';              Name = 'autofocus measures a centred window' },
    @{ File = 'server\astrodeck\focus\pipeline.py';          Pattern = 'peek_next';                       Name = 'autofocus predictor asks the engine' },
    @{ File = 'server\astrodeck\focus\approach.py';          Pattern = 'async def approach';              Name = 'one-side approach rule' },
    @{ File = 'server\astrodeck\config.py';                  Pattern = 'approach_overshoot_steps';        Name = 'approach overshoot knob' },
    @{ File = 'server\astrodeck\hub.py';                     Pattern = 'async def pier_side_now';         Name = 'flip checks the pier side before paying' },
    @{ File = 'server\astrodeck\sequence\engine.py';         Pattern = '_post_flip_focus_is_owed';        Name = 'post-flip autofocus only when owed' },
    @{ File = 'server\astrodeck\sequence\engine.py';         Pattern = '_recentre_after_unguided_focus';  Name = 're-centre after the unguided sweep' },
    @{ File = 'server\astrodeck\sequence\resume_arm.py';     Pattern = 'def resume_expected_tonight';     Name = 'warm ramp skipped for an armed resume' },
    @{ File = 'server\astrodeck\dawn_park.py';               Pattern = '_release_cooler';                 Name = 'dawn releases the cooler' },
    @{ File = 'server\astrodeck\events.py';                  Pattern = 'NIGHTLOG_RETRY_S';                Name = 'night log pauses and retries' },
    @{ File = 'server\astrodeck\sequence\engine.py';         Pattern = 'GUIDE_CALIBRATE_TIMEOUT_S';       Name = 'guide-start bound covers a calibration' }
)
foreach ($c in $checks) {
...
if ($h -notmatch '0\.3\.26') { throw "the running server is not 0.3.26: $h" }
