# Deploy 0.3.30 to the astrotown rig. Run ON the rig (elevated ssh session).
#
# What 0.3.30 carries, on top of 0.3.29 (which ran 18 minutes on 2026-09-10
# and was rolled back on the user's word): ATLAS mode in the Sky hub (the
# classic pannable survey canvas as a fourth mode; the old #/atlas link lands
# on it), and the rig session's three fixes from the 2026-09-10 night:
# consecutive dither settle failures hold a walking field
# (guide.dither_settle_fail_limit, default 2, 0 disables); the guider stops
# itself on a re-lock past guide.relock_jump_arcsec (120) or an accumulated
# guide.relock_arcsec_limit (300) inside the re-lock window (0 disables
# either); and the dawn park no longer accepts a veto from a PAUSED run (a
# running run still vetoes). The two loops (temp comp, dew) still ship
# DISABLED.
#
# All 0.3.29 gates stay. previous -> 0.3.29 keeps the rollback.
...
if ($staged.Line -notmatch '0\.3\.30') { throw "staged tree is not 0.3.30" }
$checks = @(
...
if (-not (Select-String -Path $ui.FullName -Pattern '0\.3\.30' -Quiet)) { throw "the built UI does not carry the 0.3.30 version stamp" }
Write-Output "   UI bundle carries 0.3.30"
...
if ($h -notmatch '0\.3\.30') { throw "the running server is not 0.3.30: $h" }
