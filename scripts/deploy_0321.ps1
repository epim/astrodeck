...
Write-Output "== server-only: the UI asset is unchanged from 0.3.20 =="
$hub = Join-Path $Rel "serverstrodeck\hub.py"
$hit = Select-String -Path $hub -Pattern "guide_cam, .sensor_height"
if ($hit) { Write-Output "   guide-fov from the guide sensor: present" }
else       { throw "the guide-fov fix is NOT staged" }
