...
- Seed flows (fixtures): **M31 — LRGB two-night**, **M16 — full-service night**, **Best-of-four pool night**, **NGC 7000 — Ha narrowband**, **EAA quick look** (graphs identical to the prototype's `PRESETS`).
...
Centered overlay `min(880px,100%)`, three pill tabs `TIMELINE / STORY / PLAN`.
...
- **STORY**: time column (mono 10px, right-aligned, 60px) + sentence rows; special rows `ANY` (rules, warn/danger-colored) and `BUDGET` (integration: "Ha: 4.2 h banked / 12 h goal — tonight adds ≈1 h; the session ledger resumes the remainder next clear night", `--good`).
...
Ports: `kind: flow` (cyan; exactly one run cursor travels it) or `event` (amber; may fire any time). Wiring only kind→same-kind. Per node: label · cat/color · ins/outs · params (defaults in the prototype's `DEFS`) · backend mapping:

| Node | Ports (in → out) | Compiles to / backend |
|---|---|---|
| DUSK WINDOW | → window (flow) | `Schedule` start_mode dusk/offset/stop, min_altitude_deg (`schedule.py`) |
| TARGET | arm → target (flow) | `Target` (name, ra, dec, rotation_deg) |
| SAFETY MONITOR | → unsafe (event) | existing fail-closed `SafetyMonitor`; stale ⇒ unsafe, always |
...
| DOME CONTROL | open (flow) → shutter open (flow) | **new** `Dome` device role (Alpaca Dome); slave-to-mount; on-unsafe close is non-negotiable |
...
| TARGET POOL | arm → best target (flow) | **new** scheduler mode over existing constraints (min alt, moon sep, max HA — all already in `Schedule`); strategies: best available / priority / round robin; re-evaluates on completion/floor |
| CONDITION | events → fire (event) | `Instruction` trigger/`Condition` compound (closed predicate vocabulary — never a scripting runtime) |
...
| HOLD / RESUME | pause, resume → | **new** action pair over engine pause/resume; params: while-paused (guider policy), max hold + on-timeout (Abort+park default), on-resume: re-center (solve) or trust tracking, refocus if-drifted/always/never, restore active filter (always) |
| ABORT + PARK | do → | existing abort: park, warm, reason → report |
| SESSION REPORT | session → | `report.py` append-only ledger |
...
```
Pool targets carry `pool_rank`, `min_altitude_deg`, `min_moon_sep_deg`, `max_hour_angle_h`. New TriggerKinds are **additive** to the existing closed enum: `on_clouds_in`, `on_clouds_clear`.
...
7. Engine: hold/resume with resume policy (restore filter → optional re-center → conditional refocus), calibration-queue runner, dusk-flats stage, pool scheduler, transient-weather trigger plumbed to the weather integration. All fail closed; hold has a max-hold timeout defaulting to Abort+park.
