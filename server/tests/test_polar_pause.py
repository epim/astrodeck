...
async def test_terminal_states_still_land_while_paused(terminal):
    """Suppressing every state would be worse than the bug: a run that finishes
    (or fails) while paused would stick on "paused" with no way out."""
    h = Hub()
    sess = h.polar
    sess._task = asyncio.create_task(asyncio.sleep(30))
    try:
...
async def test_sim_pause_during_the_measuring_points_holds(monkeypatch):
    ...
    try:
        ...
        assert h.polar.state["message"] == at_pause["message"], h.polar.state
