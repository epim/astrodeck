...
import type { LogLine, TroubleshootTopic } from "../types";
...
export interface Diagnosis {
  ...
  /** True when the run ended because the OPERATOR ended it (hold-to-abort).
   *  Consumers must not dress this as a failure: no red alarm, no advisory,
   *  no Help deep-link, no log excerpt hunting for a cause that never was. */
...
function userAbortDiagnosis(ctx: FailureContext): Diagnosis {
  const { framesDone, framesTotal } = ctx;
  const at =
    framesDone != null && framesTotal != null && framesTotal > 0
      ? ` at ${framesDone}/${framesTotal} frames`
      : framesDone != null
        ? ` after ${framesDone} frames`
        : "";
  return {
 ...
 *
 * `ts` is epoch SECONDS on both sides. Pure; never throws.
 */
export function runFailureLog(
  logs: readonly LogLine[] | undefined,
  startedAtS: number | null | undefined,
  limit = 5,
): LogLine[] {
  if (startedAtS == null || !logs?.length) return [];
  return logs
    .filter((l) =>
      (l.data.level === "error" || l.data.level === "warning") && l.ts >= startedAtS)
    .slice(-limit);
}
