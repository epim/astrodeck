...
| N/A (no-lost-functionality gap this ledger missed) | The classic full-sky Atlas (`views/AtlasView`) had NO DOOR in the new UI until this change. The Sky tab opens on the schematic MAP finder; the survey imagery lived only in FRAME, which is locked until the reticle is above the horizon or a target is locked; and `legacyBridge`'s `atlas` route pointed at `/sky?frame=1`, which RESUMES a framing session - so an old `#/atlas` bookmark with no session landed on the schematic finder and toasted "nothing is framed yet", which reads as "no atlas loaded". ATLAS is now a fourth mode beside MAP / FRAME / GYRO (`finder/prefs.ts SkyMode` gained `"atlas"`), mounting `hubs/sky/atlas/AtlasHost` - the same kept-as-is `components/atlas/SkyCanvas` FRAME mounts, pannable over the whole sky - and `LEGACY_VIEW_ROUTE.atlas` is `/sky?mode=atlas` | Smallest change with full parity: one renderer, one framing session, one degraded-copy module (`frame/degraded.ts`), so FRAME and ATLAS cannot disagree about what the sky is or why it is dark. Three consequences worth naming rather than discovering: (a) ATLAS suspends FRAME rather than ending it, and hides the lock/patch/reach cards while it is up, because each of those answers about a reticle that is not on screen; (b) `store.openFraming()` writes `view: "atlas"`, so entering FRAME from the Sky hub travels out through the bridge and comes back as `?mode=atlas` - `SkyHub` ignores that parameter while FRAME is already on, which is the only reason the FRAME button does not drop the user into ATLAS; (c) the degraded row on the atlas is the ROUTE to Settings > SKY PACK (with the download's live progress on it), not a third copy of the degraded sentence - `SkyCanvas` already paints that sentence twice on its own, over the empty sky and in its own banner, and the reason rides on the row's title and accessible name instead; (d) Rig > Mount's FRAME button (`rig/sheets/mount.tsx:954 openFraming(r)`) therefore now lands on ATLAS rather than on FRAME mode, which is what its own label ("Frame {id} in the sky atlas") always promised and what the classic app did - the mosaic and rotation tools are one press of the atlas's FRAME button away. `hubs/sky/atlas/AtlasHost.tsx`, `hubs/sky/SkyHub.tsx` (the ATLAS block), `next/legacyBridge.ts:58`. **AMENDED, and consequence (a) above is now the opposite of what shipped**: ATLAS is the DEFAULT mode on every device (`finder/prefs.ts`'s `DEFAULT_MODE`) rather than only ever a choice, because what a user opens the app to see is the sky with tonight's targets on it and not a reticle already aimed at one of them - the stored `astrodeck-next-sky-mode` key still beats the default in both directions, and the toolbar reads ATLAS, MAP, FRAME, GYRO. Four things follow. The atlas is POPULATED BY THE TARGETS: `useSkyModel.targets` - the same ranked, lens-filtered list MAP draws - is drawn over the canvas as the design's own label pills (`atlas/AtlasMarkers.tsx`), tap to lock, ring on the lock, and those ids are filtered out of the rows handed to `SkyCanvas` so no object carries two labels. The LOCK CARD is therefore on both modes now (the reach strip and the FRAME cluster are still MAP's alone: one lists things that are off screen, which on the atlas are on it, and the other is controls for a rectangle that is not drawn). AIM ANYWHERE: a tap that hits no marker is `SkyCanvas`'s own `onPickObject(null)` hook - the only empty-sky gesture it exposes, and it is keep-as-is so no second recogniser was added beside its own - and the RA/Dec comes off the same pointer event through `atlas/aim.ts`'s `boxToSky`, the exact inverse of `lib/atlasFov.ts`'s `skyToView`; it writes the finder's aim, the framing centre, and clears the framed object, so the reticle, IMAGE THIS PATCH, FRAME and the dome all follow one tap. And a SURVEY layer toggle lives under the stack icon, on by default, off meaning `SkyCanvas` mode `schematic` - markers, reticle and pointing footprint on the plain dark surface with not one tile fetched - stored in the existing `astrodeck-next-sky-layers` key with field-scoped writes, because that key now has two owners (the model's three overlays and the hub's survey flag) and a wholesale write from either would resurrect the other's stale value | shipped (D-SKY-5, amended) |
...
| ARCHITECTURE section 2 (as written 2026-09-10): the bare root `#/` mounts `NextApp`, the classic app lives at `#/classic`, and the classic app gets no link back ("the user types `#/`") | Since 2026-09-15 the bare root mounts the CLASSIC app again, together with every legacy view name at the root (`#/atlas`, `#/mount`, ...); the new UI keeps all six hub routes and their sheets and gains `#/next` as the alias for its home; classic Settings gained a visible `link-next-ui` link across | The owner asked for it: the classic app has since been enhanced with the unified sky atlas and the refined dome (`dc09a27c`, `components/sky/Classic*`, `cloudmap/ClassicSkyDomePanel.tsx`) and is what the rig is driven from. The choice is ONE constant, `DEFAULT_ROOT` in `ui/src/rootChoice.ts`; setting it back to `"next"` restores this row's original behaviour with no other edit, and `ui/src/__tests__/rootChoice.test.tsx` proves both settings so the constant cannot rot into a comment. `monitor` and `settings` are both a hub name and a legacy view name and the HUB wins - a live route must not break to fix a bookmark. The "add no link back" half is reversed outright: a one-way door is how a user ends up typing hashes | deliberate - 2026-09-15 |
