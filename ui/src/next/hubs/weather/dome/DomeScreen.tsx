...
//
// What this file adds is the words around it, and it is careful to add only
// words that are true of the picture underneath - see `domeOverlay.tsx` for why
// the horizon fill and the ghost tiles are written out rather than drawn.
...
import { DomeLegend, NOT_DRAWN_NOTE, windSummary } from "./domeOverlay";
    ...
    <div data-testid="wx-sky" style={COL}>
      ...
      <Card>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <SkyDomePanel pointing={pointing} target={targetAltAz} />

          <DomeLegend hasTarget={!!targetAltAz} wind={wind} />

          <Mono size={10} tone="dim">{clauses.join(" · ")}</Mono>
          <Mono size={10} tone="dim">{NOT_DRAWN_NOTE}</Mono>
        </div>
      </Card>
    </div>
  );
