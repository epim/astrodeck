#!/usr/bin/env python3
"""Assemble the DENSE AstroDeck primer from _shell.html + _sections_dense/*.html.

Sorted by filename so 11b-evening-session sorts between 11 and 12.
"""
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parent
SECTIONS_DIR = ROOT / "_sections_dense"
SHELL = ROOT / "_shell.html"
OUT = ROOT / "astrophotography-primer-dense.html"
...
for f in section_files:
    ...
    # wrap each section script in its own <script> tag (each is already an IIFE)
    scripts.append(
...
print(f"files: {[f.name for f in section_files]}")
