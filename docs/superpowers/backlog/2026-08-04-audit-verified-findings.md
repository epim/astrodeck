...
## #219 — autofocus — **CLOSED 2026-08-17**

Filed as "autofocus only works on a bright field". Re-measured at bin 1 on a
field with **1172 stars at focus**, where it failed anyway. The diagnosis was
wrong; the failure was at the bloated end of the sweep, not the faint end.

Two defects, both fixed.

**1. A sub-pixel size outranked the real minimum** (`ea4b3b4`). The sweep found
focus at 11135 (2.96 px, 1172 stars) then walked 2450 steps past its own range
chasing 0.50 / 0.51 / 0.52 px from four to twelve "stars", and gave up after
thirteen minutes. A half-flux *radius* below one pixel means the flux fits in a
single pixel — a measurement of a pixel. `_size_point` refused only `hfr <= 0`,
and a too-small number does not blur a fit, it *steers* it. `MIN_SIZE_PX` floors
it.

**2. Coarse seeds were measured at the wrong resolution** (`a115da5`).
`star_size` walks pyramid levels coarse→fine but measured every seed at FULL
resolution with an aperture of `8*k` px; on a thin ring that locks onto an arc,
so nine rings arrived as 19 then 24 "sources" and the size plateaued near 7.7 px
whether the true radius was 24 or 34. That is the wing under-measurement:
27.06 px at −350 steps, then 10.40 and 5.52 further out, which made
`curve_verdict` refuse "an arm that turns round".

Measuring each seed at the level it was FOUND at — on the binned copy seeding
already cached, via the `_measure_at_bin` that existed for escalation — is both
more correct and cheaper. 24→24.4, 34→34.6, 48→48.8, nine rings as nine
sources, at **239 ms against a 235 ms baseline**. The full-resolution route to
the same correctness cost **23×** (5362 ms), eleven times a sweep, on a Pi.
`SIZE_BIN_TRUST` handles the trap on the way: at bin *k* a radius cannot fall
below ~one binned pixel, so a 6 px star found at k=64 reported 32 px — the bin,
not the star.

A third change — screening the seed on the binned copy — was built and
**removed**: theoretically right (a 3 px probe does land in the ring's dark
hole) but sabotage showed every test stayed green without it.

Also fixed in passing: on a monotonic sweep a parabola has *a* ≈ 0 and its sign
is noise, so "flat or inverted fit" versus "minimum not bracketed" was decided
by rounding on one physical situation. Now decided from the shape.

**Reproduction kept**: `test_star_size_fragments_a_thin_donut.py` builds the
annulus a defocused star actually is and sweeps its radius — the thing #219
never had, and why a wrong diagnosis survived a month.

Hocus Focus (George Hilios) found the same class independently; its 4.0.0.5 note
restores tighter Brightness Sensitivity because looser values "admitted
noise/donut fragments that could skew autofocus on wide/defocused sweeps".
Credited in `THIRD-PARTY-NOTICES.md`.

**Not done, and not required for this**: the sweep still spans ±1400
