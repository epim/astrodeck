...
| Size | Blocker | Fix |
|---|---|---|
| L | **Player One's SDK licence has no distribution verb**, and the owner's imaging camera is a POA Poseidon-M Pro. A flashed image conveyed to a customer is redistribution. `build_release.py` already strips these via `_UNSHIPPABLE_VENDOR_DIRS`. **[r]** | Land written permission, or make first-use fetch an explicit, offline-aware product flow. Apply the same filter in `astrodeck.spec`. |
