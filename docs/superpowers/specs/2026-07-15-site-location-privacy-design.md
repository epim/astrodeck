...
- Config-write echoes: the original draft claimed `_config_payload()` was "already redacted per-principal" — the final whole-branch review proved this FALSE by live probe (a viewer could read precise coordinates via an empty `POST /api/config`, and `ConfigVersionConflict` 409 bodies leaked both site precision and auth secrets). Fixed structurally during implementation: `_config_payload(principal)` applies `_redact_site_for` internally so no call site can forget it, and the 409 conflict bodies route through BOTH `redacted()` and `_redact_site_for`. A writer holding `config.site_optics` but not `view.site_precise` gets a stripped echo of what they just wrote (tested).
