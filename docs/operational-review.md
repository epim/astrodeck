...
>
> **Update (2026-06-16, post-review fixes — committed):** The actionable P0/P1 *code* findings below have been addressed and verified (301 tests pass): NINA-mode plate-solve now routes through the working **ASTAP** solver (capture → `save_fits` → solve) and SimSolver refuses to fake-solve a real rig; **all engine device-I/O is bounded** by `asyncio.wait_for` → abort+park; the **dead-man's-switch is wall-clock-driven** (pings through pauses) and warns on a blocked LAN URL; **optional token auth** (`ASTRODECK_TOKEN`, off by default so LAN access is unchanged); **atomic resume writes**; **PHD2 reconnect + meridian-flip calibration + `Alert`/`CalibrationFailed` handling**; the **three escalation knobs are wired into the engine**; full secret redaction; bounded survey cache; and the flaky autofocus test was root-caused (unseeded sim RNG) and fixed. The **P2/P3 *new-feature* items remain roadmap** (live-stacking, flats wizard, Docker/Pi packaging + supervised launcher, precession/refraction, schema-migration framework, NINA→Rust port). **The overall verdict stands** until the automation/safety engine runs unattended on real hardware across multiple nights.
